...
public class ExcelController {
	...
	public void excelTest() throws Exception{
		...
		int rowCnt = 0;
		String[][] resultArr = new String[100][15];
		int colCount = 0;
...
//					System.out.println(cell.getStringCellValue());
		...
		for(int j=0;j<resultArr.length;j++){
			...
			vo.setCbt_em_quiz_numb( Integer.parseInt(resultArr[j][1]));  
			vo.setCbt_em_mem_answer( Integer.parseInt(resultArr[j][2])); 
			...
			vo.setCbt_answer1(resultArr[j][11]);
			vo.setCbt_answer2(resultArr[j][12]);
			vo.setCbt_answer3(resultArr[j][13]);
			vo.setCbt_answer4(resultArr[j][14]);
