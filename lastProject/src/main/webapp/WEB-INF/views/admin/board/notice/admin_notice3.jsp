...
<script>
	function go_noticeDetail(noticeCode) {
		...
		noticeForm.action = "/boardUpdateForm3?notice_code=" + noticeCode;
		noticeForm.submit();
	}
	//검색함수
	function getNoticeList(){
		...
		location.href="<%=request.getContextPath()%>/admin/notice/search3?notice_code="+notice_code+"&schType="+schType+"&schText="+schText;
	}
</script>
								...
								<span class="page"> <%
								 ...
								 %>
									<button type="button" class="btn5"
										onclick="location.href='notice3?page=<%=i%>'" title="<%=i%>페이지">
										<span><%=i%></span>
									</button> <%
 	}
 	}
